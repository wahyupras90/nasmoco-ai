...
def parse_alamat(alamat: str) -> tuple:
    ...
    Contoh input:
      "KARANGJATI RT. 002 RW. 001 KEC. TARUB KAB. TEGAL"
      "JL. TEUKU UMAR RT. 003 RW. 006 DUKUH SALAM KEC. SLAWI KAB. TEGAL"
      "JL. ASEM TIGA GANG I NO. 2 RT. 008 KEC. TEGAL BARAT KOTA TEGAL"
    
    Return: (kecamatan, kabupaten) atau (None, None) jika tidak bisa parse.
    """
    ...
    # Extract kecamatan: setelah KEC. sampai KAB./KOTA atau akhir
    kec = None
    m = re.search(r'KEC\.\s+([A-Z0-9 ]+?)(?:\s+(?:KAB\.|KOTA)|$)', alamat)
    if m:
        kec = m.group(1).strip()
    
    # Extract kabupaten: KAB. XXX atau KOTA XXX
    kab = None
...
def _parse_rs_file(filepath: str) -> pd.DataFrame:
    ...
    Handle 2 tipe file:
    - .xlsx / .xlsm : openpyxl
    - .xls HTML     : pd.read_html (format DMS lama)
    Return DataFrame dengan kolom standar.
    """
    # Coba openpyxl dulu, fallback ke read_html untuk .xls HTML
    HEADER_KEYS = {'No_Rangka', 'No. Rangka', 'Nama Sales', 'Nama_Sales', 'Tgl_DO', 'Tgl BPK'}

    try:
        df   = pd.read_excel(filepath, header=1, engine='openpyxl')
        cols = set(df.columns.astype(str).str.strip())
        # Kalau header=1 tidak menghasilkan kolom yang dikenal, coba header=0
        # (terjadi pada xlsx hasil konversi dari HTML yang sudah punya header di row 0)
    ...
    # FORMAT BARU (2026)
        ...
        rename = {
            ...
            'No. Polisi':    'no_polisi',
            'Telp GSM':      'telp_gsm',
            'Nama Customer': 'customer',   # format DMS baru
            'Nama STNK':     'customer',   # format xlsx konversi
...
def load_rs_full(folder: str) -> pd.DataFrame:
    ...
    # Parse kecamatan dan kabupaten dari alamat_stnk
    if 'alamat_stnk' in df.columns:
        parsed = df['alamat_stnk'].apply(
            lambda x: pd.Series(parse_alamat(x),
                                index=['kecamatan', 'kabupaten'])
        )
        df['kecamatan'] = parsed['kecamatan']
        df['kabupaten']  = parsed['kabupaten']
    else:
        df['kecamatan'] = None
        df['kabupaten']  = None
...
def save_to_db(db_path: str, df_um, df_inv, df_bp, df_rb,
    ...
    # Total per tabel
    for tbl in ['unitmasuk','invoice','bufferparts','rekapbulanan']:
        try:
